...
import lan.chaos.excel.common.config.ExcelProperties;
import org.springframework.stereotype.Component;

import java.io.File;

/**
 * 产物落盘：所有场景生成的 xls/xlsx/csv 统一写到配置目录（默认 target/out）。
 *
 * <p>WHY：产物必须落在被 .gitignore 忽略的 target/ 下——
 * ① 二进制文件进 git 会污染仓库且无法 diff；
 * ② 但文件又必须真实落盘，因为「能用 Excel 打开」是这类 Demo 唯一的强验证。
 */
@Component
public class OutFiles {
    ...
    public OutFiles(ExcelProperties properties) {
