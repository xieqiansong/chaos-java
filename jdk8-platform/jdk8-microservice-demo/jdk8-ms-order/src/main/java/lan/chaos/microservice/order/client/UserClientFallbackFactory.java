 ...
 *
 * <p>在 {@code @GlobalTransactional}（createWithTx）里若 user 被熔断，deductAccount 会返回降级 R，
 * order 侧据此抛异常触发全局回滚——即使分支没真正执行，也会把已写的订单分支撤销。</p>
 */
@Component
public class UserClientFallbackFactory extends AbstractFallbackFactory<UserClient> {
    ...
    protected UserClient createFallback(Throwable cause) {
        return new UserClient() {
            ...
            @Override
            public R<Void> deductAccount(Long id, BigDecimal amount) {
                return FallbackResults.degraded("ms-user#deductAccount(" + id + "," + amount + ")");
            }
        };
    }
}
