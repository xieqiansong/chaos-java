...
- **jdk8-rabbitmq-demo**：基于 Spring AMQP 2.4.x（jdk8-mq 组），覆盖 Exchange 四种类型路由（direct/topic/fanout/headers）、Publisher Confirm 生产者确认、消费者手动 Ack（basicAck/basicNack 重入队）、TTL+DLX 死信与延迟消息；rabbitmq-mock 内存 Broker 自包含 *Test + Testcontainers 真实 Broker *Test（无 Docker 优雅跳过）。详见 `jdk8-mq/jdk8-rabbitmq-demo/README.md`。
