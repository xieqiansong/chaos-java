...
| 25 | `jdk8-webflux-demo` | ✅ 完成 | WebFlux 响应式编程：Reactor（Mono/Flux + map/flatMap/filter）、背压（request(n) 按需拉取 + 策略）、RouterFunction 函数式路由、注解式响应式 Controller（对照）、WebClient 异步非阻塞调用；Netty 非阻塞服务器，内存仓储 + 内嵌端点，零外部依赖，StepVerifier/WebTestClient 双轨验证 |
| 26 | `jdk8-common` | 🟡 占位 | 公共基础模块占位，承载跨 demo 的公共工具/实体 |
...
| 29 | `jdk8-rabbitmq-demo`（jdk8-mq 组） | ✅ 完成 | **RabbitMQ（Spring AMQP 2.4.x）**：Exchange 类型路由（direct/topic/fanout/headers）、Publisher Confirm 生产者确认、消费者手动 Ack（basicAck/basicNack 重入队）、TTL+DLX 死信与延迟消息；rabbitmq-mock 内存 Broker 自包含 *Test + Testcontainers 真实 Broker *IT（无 Docker 优雅跳过）。详见 `jdk8-mq/jdk8-rabbitmq-demo/README.md`。 |
...
- **jdk8-rabbitmq-demo**：基于 Spring AMQP 2.4.x（jdk8-mq 组），覆盖 Exchange 四种类型路由（direct/topic/fanout/headers）、Publisher Confirm 生产者确认、消费者手动 Ack（basicAck/basicNack 重入队）、TTL+DLX 死信与延迟消息；rabbitmq-mock 内存 Broker 自包含 *Test + Testcontainers 真实 Broker *IT（无 Docker 优雅跳过）。详见 `jdk8-mq/jdk8-rabbitmq-demo/README.md`。
...
- **jdk8-seata-demo**：基于 Seata 1.6.x + Spring Cloud Alibaba，覆盖 AT 模式（自动 undo_log 补偿、undo_log 镜像、全局锁并发）、TCC 模式（手动 Try/Confirm/Cancel + 空回滚/幂等/悬挂语义）、SAGA 补偿链、XA 数据源代理、XID 跨线程传递；JdbcTemplate 直操作业聚焦事务概念，H2 内存数据库自包含测试，需真实 Server 的集成场景用 Assumptions 优雅跳过。
...
- **jdk8-webflux-demo**：基于 Spring Boot 2.7 + spring-boot-starter-webflux（Netty 非阻塞，未引 MVC），根包 `lan.chaos.webflux`，覆盖响应式编程全链路五能力：`reactor`（MonoFluxBasics 演示 Mono/Flux + map/flatMap/filter；BackpressureDemo 演示 request(n) 按需拉取与 onBackpressure* 策略）、`router`（ProductRouter + ProductHandler 的 RouterFunction 函数式端点）、`controller`（ProductController 注解式响应式 Controller，路径前缀 `/api/annotated` 与函数式路由对照）、`webclient`（ProductWebClient 异步非阻塞调用本应用内嵌端点）。内存仓储 + 内嵌端点零外部依赖；测试双轨：Reactor 语义用 `StepVerifier`、端点用 `WebTestClient`（RANDOM_PORT），均进 `mvn test`；控制台 `DemoApp.main` 分节打印「输入→输出」。详细见 `jdk8-webflux-demo/README.md`。

- **jdk8-office-tech**：办公文档处理组，3 个子模块——`jdk8-excel-demo`（POI / EasyExcel / Hutool 三体系横评，含版本守门防 CVE 被 BOM 吞、HSSF 65536 行与 `CellStyle` 64000 上限等硬约束）、`jdk8-word-demo`（文档生成与处理）、`jdk8-pdf-demo`（PDFBox 处理）。配套笔记：[Excel](https://github.com/xieqiansong/chaos-notes/blob/main/notes/Excel处理学习记录.md) / [Word](https://github.com/xieqiansong/chaos-notes/blob/main/notes/Word处理学习记录.md) / [PDF](https://github.com/xieqiansong/chaos-notes/blob/main/notes/PDF处理学习记录.md)。
...
- **补充计划**：详见仓库根 [`TECH-INDEX.md`](../TECH-INDEX.md#待补技术点与建设优先级) 的「待补技术点与建设优先级」章节，当前待补的 🟡 部分覆盖项和 ❌ 待补项均已规划具体 demo 和建设顺序。

## 扩展阅读

- **技术点索引与待补计划（全仓库）**：仓库根 [`TECH-INDEX.md`](../TECH-INDEX.md)，按技术域组织，覆盖全部 5 个 JDK 平台，含未完成项清单与建设优先级。
