...
| 7 | `jdk8-microservice-demo` | 🚧 P1/P2 完成 / P3 进行中 | **企业级 Spring Cloud Alibaba 微服务学习项目（形态二：多进程）**：模块分明（common 层 6 模块 + 业务服务 4 模块）、可观测（统一日志 + traceId）、多数据源（PostgreSQL 主 / MySQL 副）、统一响应异常处理、分层测试。规划与开发计划见 `jdk8-microservice-demo/开发计划.md`，代码按 P0→P6 路线图分阶段落地（P0 骨架 / P1 基础 / **P2 熔断限流优先** / P3 分布式事务优先 / P4 安全 …） |
