...
| 3 | `jdk8-mybatis-plus-demo` | ✅ 完成 | MyBatis-Plus 高阶：条件构造器 / 分页（单表+联表）/ 逻辑删除+乐观锁+自动填充 / 多租户隔离 / 动态表名分表 / 字段透明加密（AES），以单元测试为核心验证，H2 内存库零外部依赖 |
...
| 19 | 🟡 中 | 本地缓存（Caffeine / Guava Cache） | ❌ 待补 | 建议补 demo |
...
- 🟡 中 本地缓存（Caffeine / Guava Cache / Ehcache） — ❌ 待补
- ⚪ 低 多级缓存（本地 + Redis） — ❌ 待补
